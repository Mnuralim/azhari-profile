...
const EditRiwayatPekerjaan = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const res = await axios.get(`https://colorful-calf-helmet.cyclic.app/get-workhistory/${params.id}`);
  ...
  const handleForm = async (e) => {
    ...
    try {
      const response = await axios.put(`https://colorful-calf-helmet.cyclic.app/update-workhistory/${params.id}`, formData);
    ...
    <section className="relative flex min-h-screen mt-10 mb-20 lg:mt-20" id="contact">
      <div className="w-full">
        <h1 className="mt-16 text-4xl font-bold text-center lg:text-5xl">
          Edit <span className="text-indigo-500">Riwayat Pekerjaan</span>
        </h1>
...
export default EditRiwayatPekerjaan;
