...
export default function CustomPaginationActionsTable() {
  ...
  React.useEffect(() => {
    const getData = async () => {
      const res = await fetch("https://colorful-calf-helmet.cyclic.app/get-community", { cache: "no-store" });
  ...
  const handelDelete = async (id) => {
    ...
    console.log(id);
    if (confirmed) {
      const data = await axios.delete(`https://colorful-calf-helmet.cyclic.app/delete-community/${id}`);
      if (data.status == 200) {
        alert("Sukses");
    ...
    <section className="mt-20 overflow-x-hiddenbg-slate-50 dark:bg-slate-800 dark:text-slate-50 md:mx-10">
      <div>
        <h1 className="mb-10 text-4xl font-bold text-center mt-14 text-slate-700 lg:text-5xl lg:mt-24 dark:text-slate-50">
          Daftar <span className="text-indigo-500">Komunitas</span>
        </h1>
      </div>
      <TableContainer>
        <Button onClick={handleRefresh} className="font-bold text-white bg-indigo-500 hover:bg-indigo-800">
          Refresh
        </Button>
          ...
          <TableBody>
            {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row) => (
              <TableRow key={row._id}>
                ...
                <TableCell className="capitalize cursor-pointer dark:text-slate-50">
                  <Button onClick={() => handelDelete(row._id)} className="px-3 py-1 font-bold text-white bg-pink-500 cursor-pointer hover:bg-purple-500">
                    Hapus
                  </Button>
                </TableCell>
              </TableRow>
