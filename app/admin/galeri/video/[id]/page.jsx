...
const UpdateVideoForm = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const data = await axios.get(`https://colorful-calf-helmet.cyclic.app/get-gallery-video/${params.id}`);
  ...
  const handleForm = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.put(`https://colorful-calf-helmet.cyclic.app/update-gallery-video/${params.id}`, {
        title: title,
        url: url,
      });
    ...
    <section className="relative flex min-h-screen mt-10 mb-20 lg:mt-20" id="contact">
      <div className="w-full">
        <h1 className="mt-16 text-4xl font-bold text-center lg:text-5xl">
          Update <span className="text-indigo-500">Galeri Video</span>
        </h1>
